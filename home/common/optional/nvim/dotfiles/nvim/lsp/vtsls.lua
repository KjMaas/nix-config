...
-- Resolve the vue-language-server package path dynamically from the nix store
-- so it doesn't break on package updates.
local vue_bin = vim.fn.exepath("vue-language-server")
local vue_store_path = vue_bin:match("(/nix/store/[^/]+)")
local vue_language_server_path = vue_store_path
    and (vue_store_path .. "/lib/language-tools/packages/language-server/package.json")
    or ""
